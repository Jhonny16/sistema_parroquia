...
function cargarClienteId(p_nombreCombo) {
    ...
    (
        ...
        if (datosJSON.estado === 200) {
            var html = "";
            if($("#sesion_rol_id").val() == '4'){
                html += '<option value="">Seleccione Cliente</option>';
                $.each(datosJSON.datos, function (i, item) { //each para recorrer todos los elementos de array
                    if($("#sesion_dni").val() == item.per_iddni){
...
function clear_reserv(){
    if($("#sesion_rol_id").val() == '4'){
        window.location.href = '../vista/reserva_cliente_vista.php';
    }else{
        window.location.href = '../vista/reserva_vista_externa.php';
    }
