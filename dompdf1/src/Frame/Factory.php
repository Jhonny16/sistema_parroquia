...
class Factory
{
     ...
     * @param $dompdf Dompdf The dompdf1 instance
     *
    ...
    static function decorate_frame(Frame $frame, Dompdf $dompdf, Frame $root = null)
    {
        ...
        if ($display === "list-item") {
            ...
            if ($parent_node) {
                if (!$parent_node->hasAttribute("dompdf1-children-count")) {
                    $xpath = new DOMXPath($xml);
                    $count = $xpath->query("li", $parent_node)->length;
                    $parent_node->setAttribute("dompdf1-children-count", $count);
                }

                if (is_numeric($node->getAttribute("value"))) {
                    $index = intval($node->getAttribute("value"));
                } else {
                    if (!$parent_node->hasAttribute("dompdf1-counter")) {
                        $index = ($parent_node->hasAttribute("start") ? $parent_node->getAttribute("start") : 1);
                    } else {
                        $index = (int)$parent_node->getAttribute("dompdf1-counter") + 1;
                    }
                }

                $parent_node->setAttribute("dompdf1-counter", $index);
                $bullet_node->setAttribute("dompdf1-counter", $index);
            }

            $new_style = $dompdf->getCss()->create_style();
            $new_style->display = "-dompdf1-list-bullet";
